...
describe("FinanceSummarySection", () => {
  ...
  it("shows real totals and hints, color-coding gold P&L and any outstanding debt", () => {
    ...
    expect(screen.getByText("20 phân", { exact: false })).toBeInTheDocument()
    const goldPL = screen.getByText("lỗ " + formatMoney(1_000_000))
    expect(goldPL).toHaveClass("font-bold")
    expect(goldPL).toHaveStyle({ color: "var(--ob-color-expense)" })
