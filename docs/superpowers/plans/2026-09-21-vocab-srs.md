...
```ts
      setRound((prev) => {
        ...
        // Không reset lastMissedIds về [] ở đây — giữ nguyên prev.lastMissedIds (qua spread ...prev)
        // vì effect báo SRS bên dưới có thể chưa kịp chạy để đọc/xoá nó (nhiều tick liên tiếp có
        // thể bị React gộp vào 1 lần commit duy nhất, ví dụ khi tab bị throttle chạy nền, hoặc khi
        // test dùng 1 lần vi.advanceTimersByTime(80000) gộp rất nhiều tick — reset sớm ở đây sẽ
        // xoá mất id đang chờ báo mà effect chưa kịp đọc).
        ...
        return {
          ...
          // Cộng dồn vào prev.lastMissedIds (KHÔNG ghi đè) — cùng lý do như trên: nếu nhiều tick
          // liên tiếp đều rơi từ trước khi effect kịp chạy 1 lần để báo & xoá, ghi đè sẽ làm mất
          // toàn bộ id của các tick trước đó, chỉ còn sót lại đúng 1 id của tick cuối cùng.
...
```ts
  ...
  useEffect(() => {
    if (round.lastMissedIds.length > 0) {
      ...
      // Xoá về [] sau khi báo — bắt buộc phải có bước này vì updater ở trên giờ CỘNG DỒN
      // (không ghi đè) vào lastMissedIds; thiếu bước xoá này, id đã báo rồi sẽ nằm lại mãi trong
      // mảng và bị báo lại (trùng lặp) mỗi khi có thêm 1 từ rơi mới ở tick sau. Guard
      // `length > 0` chặn effect tự gọi lại vô hạn: setRound tạo mảng [] MỚI (khác reference) nên
      // effect chạy thêm đúng 1 lần nữa sau khi xoá, nhưng lần đó length === 0 nên không làm gì.
      // eslint-disable-next-line react-hooks/set-state-in-effect -- xoá lastMissedIds đã báo xong, tránh báo lại khi có tick mới
      setRound((prev) => ({ ...prev, lastMissedIds: [] }))
    }
